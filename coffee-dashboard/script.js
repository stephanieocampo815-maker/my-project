...
function updateKPIs(data) {
  ...
  let totalCost = 0;
  data.forEach(r => {
    const revenue = r.Price * r.UnitsSold;
    const cost = r.CostPerUnit * r.UnitsSold;
...
function updateCharts(data) {
  ...
  data.forEach(r => {
    ...
    costBreakdown.Operating += r.OperatingExpenses || 0;
    costBreakdown.Marketing += r.MarketingExpenses || 0;

    // product sales
    productSales[r.Product] = (productSales[r.Product] || 0) + r.UnitsSold;
  });
